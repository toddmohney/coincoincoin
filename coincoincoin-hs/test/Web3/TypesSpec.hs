...
import CoinCoinCoin.Congress.Events.Types (Voted(..), VotedReturnValues(..))
import Web3.Types
...
spec =
    describe "Event's FromJSON instance" $
        it "decodes Event JSON" $ do
            let (Right e) = AE.eitherDecodeStrict votedEventPayload :: Either String (Event Voted VotedReturnValues)
